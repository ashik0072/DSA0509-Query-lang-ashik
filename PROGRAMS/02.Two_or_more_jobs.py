...
# Create a DataFrame
employee_jobs_df = pd.DataFrame(data)

# Group by EMPLOYEE_ID and count the number of unique JOB_IDs
employee_job_counts = employee_jobs_df.groupby('EMPLOYEE_ID')['JOB_ID'].nunique()

# Select EMPLOYEE_IDs with two or more jobs
multiple_jobs_employees = employee_job_counts[employee_job_counts >= 2].index

# Display the result
print("Employees with two or more jobs:")
print(multiple_jobs_employees)
